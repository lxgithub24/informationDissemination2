...
try:
    ...
    conn = MySQLdb.connect (host="218.76.52.45" , user="root", passwd="zijidelu", db="opensns681", port=2501, use_unicode=True, charset="utf8")  
    cursor = conn.cursor ()  
    ...
    #     infofilenew.write(str(id)+" "+username[1]+" "+username[2]+" "+username[3])
        ...
        cursor.execute ("SELECT id from opensns_weibo where uid=" + str(id) + " and (create_time > " + str(int(username[4])-200) + " AND " + "create_time < " + str(int(username[4])+200)+")")  
        rows = cursor.fetchall() 
        if len(rows) == 0:
