...
conn = MySQLdb.connect (host = "localhost", user = "root", passwd = "zijidelu", db = "opensns681", use_unicode=True, charset="gbk")  
cursor = conn.cursor ()  
while username!="":
...
#     print id
    ...
    cursor.execute ("SELECT id from opensns_weibo where uid="+str(id)+" and create_time=1")  
    rows = cursor.fetchall() 
    if len(rows)==0:
        cursor.execute("insert into opensns_weibo(uid,content,create_time,comment_count,status,is_top,type,data,repost_count,`from`) values("+str(id)+",'"+username[1]+"', 1453513417,"+username[3]+", 1, 0, 'feed', 'a:0:{}'," +username[2]+",'')")
        conn.commit()
    username = infofile.readline().rstrip('\r\n')
